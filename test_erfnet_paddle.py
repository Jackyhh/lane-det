...
# import paddle.static.cuda_places as cuda_places
# import paddle.fluid as fluid
from paddle.io import DataLoader
# import paddle.utils.unique_name.guard as guard
import paddle
...
def main():
    ...
    # get places
    places = paddle.static.cuda_places(0)
    ...
    with paddle.utils.unique_name.guard():
        ...
        if args.resume:
            ...
            checkpoint, _ = paddle.load(args.resume)
            model.load_dict(checkpoint)
            print("=> checkpoint loaded successfully")
        else:
            print(("=> loading checkpoint '{}'".format('trained/ERFNet_trained')))
            # model = paddle.Model(your_model) 
            # model.load_dict('trained/ERFNet_trained.pdparams')
            model_state_dict = paddle.load("trained/ERFNet_trained.pdparams")
...
def validate(test_loader, model):
    ...
    for i, (input, img_name, img_size) in enumerate(test_loader):
        ...
        print('tensor_output: ', tensor_output.shape)
        output = tensor_output.numpy()
